...
class SensorSuite:
    ...
    pitch: float
    imu: MPU6050Readings
    # --- ADDED: Side camera image ---
    side_camera_image: Optional[pygame.Surface] = None
    # ---
    x: float = 0.0
    y: float = 0.0
...
class Vision:
    ...
    def _clear_cache(self):
        ...
        # --- ADDED: Cache for side pole ---
        self._best_green_pole_side: Optional[Dict] = None
        self._found_best_green_pole_side: bool = False
        # ---
    ...
    # --- MODIFIED: Methods for Green Pole (Marker) - SIDE CAMERA ---
    def get_best_pole_side(self) -> Optional[Dict]:
        """
        Finds the 'best' green pole for the SIDE camera.
        Just takes the largest green blob found.
        """
        if not self._found_best_green_pole_side:
            self._found_best_green_pole_side = True
            if not self.green_blobs:
                self._best_green_pole_side = None
            else:
                # --- MODIFIED: Removed the shape filter. Just take the largest. ---
                potential_poles = self.green_blobs
                # ---
                if not potential_poles:
                    self._best_green_pole_side = None
                else:
                    self._best_green_pole_side = max(potential_poles, key=lambda b: b['area'])
        return self._best_green_pole_side

    def is_pole_visible_side(self) -> bool: return self.get_best_pole_side() is not None
    def get_pole_center_x_side(self) -> Optional[float]:
        pole = self.get_best_pole_side(); return pole['center_x'] if pole else None
    def get_pole_apparent_width(self) -> float: # Renamed from height for clarity
        pole = self.get_best_pole_side(); return pole['width'] if pole else 0.0
    # ---
